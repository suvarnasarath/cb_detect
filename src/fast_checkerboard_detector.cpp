...
{
  cv_bridge::CvImageConstPtr image_bridge = cv_bridge::toCvShare(message, "bgr8");
  const cv::Mat image = image_bridge->image;
  cv::Mat Image_mono(image.size(), CV_8UC1);

  cv::cvtColor(image,Image_mono,CV_BGR2GRAY);

  std::cerr<< grid_size_x << ", " << grid_size_y << std::endl;

  //limitROI(image);

  //ROS_INFO_STREAM("ROI x: " << roi_.x << " y: " << roi_.y << " width: " << roi_.width << " height: " << roi_.height);

  bool found_chessboard = cv::findChessboardCorners(Image_mono, cv::Size(grid_size_x, grid_size_y), corners_,
  	cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FILTER_QUADS |cv::CALIB_CB_NORMALIZE_IMAGE);
  std::cerr << found_chessboard << std::endl;

  cv::imwrite( "/home/sarath/out_Image.jpg", Image_mono );
  fixCorners();
  updateROI();
  ...
  if(found_chessboard)
  {
    ...
    cv::cornerSubPix(Image_mono, corners_, cv::Size(4, 4), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30, 0.01));
